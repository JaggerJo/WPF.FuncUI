...
module private Helpers =
    let wrappedGetter<'view, 'value>(func: 'view -> 'value) : FrameworkElement -> obj =
        let wrapper (control: FrameworkElement) : obj =
            let view = control :> obj :?> 'view
            let value = func view
            value :> obj
        wrapper

    let wrappedSetter<'view, 'value>(func: 'view * 'value -> unit) : FrameworkElement * obj -> unit =
        let wrapper (control: FrameworkElement, value: obj) : unit =
            let view = control :> obj :?> 'view
...
type AttrBuilder<'view>() =
    ...
    //static member CreateProperty<'value>(property: DependencyProperty, value: 'value, comparer) : IAttr<'view> =
    //    AttrBuilder<'view>.CreateProperty(Accessor.AvaloniaProperty property, value :> obj, comparer, ValueNone)

    /// Create a Property Attribute for an Avalonia Property
    static member CreateProperty<'value>(property: DependencyProperty, value: 'value, comparer, ?defaultValueFactory: (unit -> 'value)) : IAttr<'view> =
        let objFactory = 
            match defaultValueFactory with 
            | Some defaultValueFactory -> ValueSome (fun () -> defaultValueFactory() :> obj) 
            | None -> ValueNone
    ...
    //static member CreateProperty<'value>(property: DependencyProperty<ITemplate<_>>, value: 'value, comparer) : IAttr<'view> =
    //   AttrBuilder<'view>.CreateProperty(Accessor.AvaloniaProperty property, value :> obj, comparer, ValueNone)

    // Create a string-type Property Attribute for an Avalonia Property of type obj
    //static member CreateProperty<'value>(property: DependencyProperty, value: string, comparer) : IAttr<'view> =
    //    AttrBuilder<'view>.CreateProperty(Accessor.AvaloniaProperty property, value :> obj, comparer, ValueNone)
    ...
    /// </summary>
    static member CreateContentSingle(property: DependencyProperty, singleContent: IView option) : IAttr<'view> =
    ...
    /// </summary>
    static member CreateContentMultiple(property: DependencyProperty, multipleContent: IView list) : IAttr<'view> =
    ...
    /// </summary>
    //static member CreateSubscription<'arg>(property: DependencyProperty, func: 'arg -> unit, ?subPatchOptions: SubPatchOptions) : IAttr<'view> =
    //    // subscribe to avalonia property
    //    // TODO: extract to helpers module
    //    let subscribeFunc (control: FrameworkElement, _handler: 'h) =
    ...
    /// </summary>
    //static member CreateSubscription<'arg when 'arg :> RoutedEventArgs>(property: FrameworkElement, func: 'arg -> unit, ?subPatchOptions: SubPatchOptions) : IAttr<'view> =
    //    // subscribe to avalonia property
    //    // TODO: extract to helpers module
    //    let subscribeFunc (control: FrameworkElement, _handler: 'h) =
    //        let cts = new CancellationTokenSource()
    ...
    /// </summary>
    static member CreateSubscription(routedEvent: RoutedEvent, func: 'arg -> unit, ?subPatchOptions: SubPatchOptions) : IAttr<'view> =
        // subscribe to avalonia property
        // TODO: extract to helpers module
        let subscribeFunc (control: FrameworkElement, _handler: 'h) =
            let cts = new CancellationTokenSource()
    ...
    /// </summary>
    static member CreateSubscription<'arg>(name: string, factory: FrameworkElement * ('arg -> unit) * CancellationToken -> unit, func: 'arg -> unit, ?subPatchOptions: SubPatchOptions) =
        // TODO: extract to helpers module
        // subscribe to event
        let subscribeFunc (control: FrameworkElement, _handler: 'h) =
            let cts = new CancellationTokenSource()
