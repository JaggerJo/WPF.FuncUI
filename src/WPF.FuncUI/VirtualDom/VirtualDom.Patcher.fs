...
module internal rec Patcher =
    ...
    let private patchSubscription (view: FrameworkElement) (attr: SubscriptionDelta) : unit =
        let subscriptions =
    ...
    let private patchProperty (view: FrameworkElement) (attr: PropertyDelta) : unit =
    ...
    let private patchContentMultiple (view: FrameworkElement) (accessor: Accessor) (delta: ViewDelta list) : unit =
                ...
                delta |> Seq.iteri (fun index viewElement ->
                    ...
                    if index + 1 <= collection.Count then
                        ...
                        if shouldPatch item viewElement then
                            // patch
                            match item with
                            | :? FrameworkElement as control -> patch(control, viewElement)
                            | _ ->
                                // replace
    ...
    let private patchContentSingle (view: FrameworkElement) (accessor: Accessor) (viewElement: ViewDelta option) : unit =

        let patch_avalonia (property: DependencyProperty) =
            match viewElement with
            | Some viewElement ->
                let value = view.GetValue(property)

                if shouldPatch value viewElement then
                    Patcher.patch(value :?> FrameworkElement, viewElement)
                else
    ...
    let private patchContent (view: FrameworkElement) (attr: ContentDelta) : unit =
    ...
    let patch (view: FrameworkElement, viewElement: ViewDelta) : unit =
    ...
    let create (viewElement: ViewDelta) : FrameworkElement =
        let control =
